...
# 1. 获取当前脚本目录 (Whole_heart_segmentation_junzhe)
current_dir = os.path.dirname(os.path.abspath(__file__))

# 2. 获取上一级目录 (即包含 cineCMR_SAM 的目录)
parent_dir = os.path.dirname(current_dir)

# 3. 把上一级目录加进去，这样 Python 就能看到“邻居”cineCMR_SAM 了
...
def train_loop(model: torch.nn.Module,  ##moldel to be trained   SAM
               data_loader_train: list,  #list 包含SAX LAX
               optimizer: torch.optim.Optimizer,
               epoch: int, 
               loss_scaler,  ##防止训练时的梯度下溢
               args=None,
               inputtype = None):
    ...
    if args.turn_zero_seg_slice_into is not None:
        criterionBCE = torch.nn.CrossEntropyLoss(ignore_index=10)   ##跳过不改变的slice
    ...
    for i in range(len(data_loader_train)):     ##外层循环SAX和LAX两个方向的slice
        ...
        for data_iter_step, batch in enumerate(metric_logger.log_every(current_data_loader ,args.print_freq, header)):
            with torch.cuda.amp.autocast():##向前传播时自动混合精度
